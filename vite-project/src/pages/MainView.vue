      ...
      >
        ...
        >
          <div class="flex flex-wrap gap-3">
            <el-button
              ...
              :class="
                isVisitor
                  ? 'px-6 py-3 text-base sm:px-8 sm:py-4 sm:text-lg shadow-lg ring-2 ring-primary/30'
                  : ''
              "
            >
        ...
        >
          ...
          >
            <div>
              ...
              <p
                v-if="enhancementControlsDisabled"
          ...
          <div class="space-y-2">
            ...
            <div
              v-if="audioEnhancementStore.loading"
              ...
              >
                <div class="flex flex-col text-left">
                  <div class="flex items-center gap-2">
                    ...
                    <el-tag v-if="mode.recommended" size="small" type="success"
                      >推荐</el-tag
                    >
                  </div>
        ...
        >
          <template v-if="transcripts.length">
            ...
            >
              <header
                class="flex items-start justify-between text-xs text-slate-500 gap-3"
              >
                <div class="flex flex-wrap items-center gap-2">
                  <p class="font-medium text-slate-700">
                    {{ segment.speaker ?? "未知说话人" }}
                  </p>
                  <time class="text-[11px] text-slate-500">{{
                ...
                >
                  ...
                  <span
                    ...
                    :title="segment.commandMatch.command"
                  >
                    {{ segment.commandMatch.command }}
                  </span>
                  <span
                    class="text-emerald-600"
                    v-if="typeof segment.commandMatch.score === 'number'"
                  >
              ...
              <p
                class="text-sm mt-1"
                :class="
                  segment.finalized ? 'text-slate-800' : 'text-slate-500 italic'
                "
              >
                {{ segment.text }}
              </p>
...
const speakerToggleLoading = computed(
  () => systemSettingsStore.loading || systemSettingsStore.updating,
);
const speakerToggleDisabled = computed(
  () => !systemSettingsStore.initialized || speakerToggleLoading.value,
);
...
const enhancementStatusText = computed(() => {
  ...
  const noiseModeId =
    session.noiseMode ?? audioEnhancementStore.selectedNoiseMode;
  const noiseModeLabel =
    noiseModeOptions.value.find((item) => item.id === noiseModeId)?.label ??
    noiseModeId;
  const strength =
    typeof session.noiseStrength === "number"
      ? session.noiseStrength.toFixed(1)
      : selectedNoiseStrength.value.toFixed(1);
  const dereverbText =
...
async function handleSpeakerToggle(value: boolean) {
  ...
  if (userStore.isAuthenticated) {
    ...
    ElMessage({
      type: "success",
      message: `已${next ? "启用" : "关闭"}说话人识别（仅当前会话生效）`,
      showClose: true,
    });
  }
}
